...
public class EditFlight2 extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					EditFlight2 frame = new EditFlight2();
...
ImagePanel panel = new ImagePanel(new ImageIcon("E:\\New folder\\AirlineFlightManagementSystem\\1523.jpg").getImage());
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				{
					InternationalFlightSchedule.table.setValueAt(flightID.getText(), InternationalFlightSchedule.table.getSelectedRow(), 0);
					InternationalFlightSchedule.table.setValueAt(flightTime.getText(), InternationalFlightSchedule.table.getSelectedRow(), 1);
					InternationalFlightSchedule.table.setValueAt(flightDate.getText(), InternationalFlightSchedule.table.getSelectedRow() , 2);
					InternationalFlightSchedule.table.setValueAt(departCity.getText(), InternationalFlightSchedule.table.getSelectedRow() , 3);
					InternationalFlightSchedule.table.setValueAt(landCity.getText(), InternationalFlightSchedule.table.getSelectedRow() , 4);
					InternationalFlightSchedule.table.setValueAt(economySeats.getText(), InternationalFlightSchedule.table.getSelectedRow() , 5);
					InternationalFlightSchedule.table.setValueAt(businessSeats.getText(), InternationalFlightSchedule.table.getSelectedRow() , 6);
					InternationalFlightSchedule.table.setValueAt(distance.getText(), InternationalFlightSchedule.table.getSelectedRow() , 7);
					
					sqlconnect connection = new sqlconnect();
				    Connection con = connection.connect();
				    String query = "UPDATE internationalflights set flightID = '"+flightID.getText()+"', time = '"+flightTime.getText()+"', departureCity = '"+departCity.getText()+"', landingCity = '"+landCity.getText()+"', economySeats = "+economySeats.getText()+", businessSeats = "+businessSeats.getText()+", timeduration = "+distance.getText()+" WHERE flightID = '"+flightID.getText()+"'";
					Statement st=null;
					try {
	      	 		...
	      	 		internationalFlight objnew=new internationalFlight(flightID.getText() , flightTime.getText() , flightDate.getText() , departCity.getText() , landCity.getText() , economySeats.getText()
	      	 				, businessSeats.getText() , distance.getText());
	      	 		InternationalFlightSchedule.flightList2.set((InternationalFlightSchedule.table.getSelectedRow()), objnew);
